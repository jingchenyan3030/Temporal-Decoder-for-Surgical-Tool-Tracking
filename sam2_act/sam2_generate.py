...
def save_results(results, save_dir, orig_img_folder=None, annotations=None):
    ...
    for frame_idx, frame_results in results.items():
        ...
        mask_npy_path = os.path.join(save_dir, f"frame_{frame_idx+1:03d}_mask.npy")
        np.save(mask_npy_path, merged_mask.astype(np.uint8))
        ...
        for action in actions:
            ...
            for case_name in os.listdir(action_dir):
                ...
                for entry in kpt_entries:
                    ...
                    if not annotations or not obj_to_class:
                        print(f"[SKIP] {case_name}/{video_name}: no valid annotations.")
                        del inference_state
