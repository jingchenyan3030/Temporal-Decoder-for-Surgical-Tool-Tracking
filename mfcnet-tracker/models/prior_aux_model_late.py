...
class MultiFrameNetBasic(MultiFrameNetBase):
    def __init__(self, num_classes, num_frames, has_base_perframe_model_trained=False, with_optflow=False, with_depth=False, with_coarse=False, with_sam_mask=False, with_aux_mask_head=False):
        super(MultiFrameNetBasic, self).__init__(num_classes, num_frames, has_base_perframe_model_trained, with_optflow, with_depth, with_coarse, with_sam_mask, with_aux_mask_head)
        ...
        self.heatmap_head = nn.Conv2d(self.num_frames * self.num_classes, self.num_classes, kernel_size=1, stride=1, padding=0, bias=False)
        if self.with_aux_mask_head:
            self.aux_mask_head = nn.Conv2d(self.num_frames * self.num_classes, 1, kernel_size=1, stride=1, padding=0, bias=False)
...
class TernausNetMultiBasic(nn.Module):
    def __init__(self, num_classes, num_frames, pretrained=True, loadpath=None, optflow_inputs=False, depth_inputs=False, with_coarse=False, with_sam_mask=False, with_aux_mask_head=False): 
        ...
        self.multiframe_net = MultiFrameNetBasic(self.num_classes, self.num_frames, has_base_preframe_model_trained,
                                            with_optflow=self.optflow_inputs, with_depth=self.depth_inputs, with_coarse=self.with_coarse, with_sam_mask=self.with_sam_mask, with_aux_mask_head=self.with_aux_mask_head)
    ...
    def forward(self, x, optflow=None, depth=None,coarse=None, sam_masks=None):
        ...
        if isinstance(y_output, dict):
            heatmap_logits = y_output['heatmap']
            aux_mask_logits = y_output['aux_mask']
            return {
                'heatmap': torch.sigmoid(heatmap_logits),
                'aux_mask': aux_mask_logits
            }
        return torch.sigmoid(y_output)
