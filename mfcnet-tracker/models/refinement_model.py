"""
PyTorch module for UNet and its parts. 
Most code snippets taken from TernausNet github repository. https://github.com/ternaus/robot-surgery-segmentations 
"""
...
class DecoderBlock(nn.Module):
    ...
    def forward(self, x):
        return self.block(x)
class TernausNet16_r(nn.Module):
    def __init__(self, num_classes=2, num_filters=32, pretrained=False, in_channels=6):
        super().__init__()
...
class SingleFrameRefineTernaus(nn.Module):
    def __init__(self, use_mask=True, pretrained=True):
        super().__init__()
        self.use_mask = use_mask

        in_channels = 6 if use_mask else 5

        self.refine_model = TernausNet16_r(
            num_classes=2,
            num_filters=32,
    ...
    def forward(self, image, coarse, mask=None):
        if self.use_mask:
            if mask is None:
                raise ValueError("mask is required when use_mask=True")
            x = torch.cat([image, coarse, mask], dim=1)   # [B,6,H,W]
        else:
            x = torch.cat([image, coarse], dim=1)         # [B,5,H,W]

        hr = self.refine_model(x)
        return hr