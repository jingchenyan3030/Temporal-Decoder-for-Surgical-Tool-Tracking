    ...
    for epoch in range(start_epoch, args.num_epochs + 1):
        ...
        try:
            ...
            if args.global_rank == 0:
                ...
                with torch.no_grad():
                    if args.prediction_task != 'keypoint_heatmap':
                        val_out = validate(val_dataloader, model, args, logger, writer, epoch, optflow_model=optflow_model)
                    else:
                        val_out = validate(val_dataloader, model, args, logger, writer, epoch, optflow_model=optflow_model)
            ...
            logger.exception(f"[Rank {args.global_rank}] Exception at epoch {epoch}: {e}")
            if dist.is_initialized():
                dist.destroy_process_group()
            raise
            return
