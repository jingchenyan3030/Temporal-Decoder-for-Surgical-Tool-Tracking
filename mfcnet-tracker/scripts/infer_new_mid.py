...
def save_points_prediction(args, img_path, keypoints_dict, input_rgb):
    ...
    if args.dataset == 'ACT':
        out_dir = Path(args.data_dir).parent / "act_test_multiframe_mid" / out_rel
    else:
        out_dir = Path(args.data_dir).parent / "0923_test_multiframe_mid" / out_rel
            ...
            if step < len(file_names):      
                if step % args.save_output_freq == 0:
                    ...
                    for b in range(end - step):
                        ...
                        if args.dataset == "ACT":
                            base_pred_root = Path(args.data_dir).parent / "act_test_multiframe_mid"
                        else:
                            base_pred_root = Path(args.data_dir).parent / "0923_test_multiframe_mid"
...
def main_worker(args): 
    ...
    if args.dataset == 'ACT':
        if args.action == 'dissect':
            ...
            3: {"name": "contact", "topk": 5,"max_keep":4, "threshold": 0.3}
             }
        else:
            CHANNEL_CONFIGS = {
            1: {"name": "tip", "topk": 5, "max_keep": 2, "threshold": 0.05},
            2: {"name": "anchor", "topk": 5, "max_keep": 1, "threshold":0.05},
        ...
        if args.action == 'dissect':
            ...
            CHANNEL_CONFIGS = {
            1: {"name": "tip", "topk": 5, "max_keep": 2, "threshold": 0.1},
            2: {"name": "anchor", "topk": 5, "max_keep": 1, "threshold":0.1},
            3: {"name": "contact", "topk": 5,"max_keep":4, "threshold": 0.5}
            }
