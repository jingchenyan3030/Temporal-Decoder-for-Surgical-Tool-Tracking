...
import sys
sys.path.append('.')
from src.dataloader import get_MICCAI2017_dataset_filenames, get_JIGSAWS_dataset_filenames, get_ACT_dataset_filenames, get_KPT_dataset_filenames
...
class ImagePairsDataset(Dataset):
    ...
    def __getitem__(self, idx):
            ...
            file2 = Path(self.filenames[idx])
            video_dir = file2.parent

            video_frames = natsorted(list(video_dir.glob("*.png")), key=str)
            local_idx = video_frames.index(file2)

            if local_idx == 0:
                file1 = file2
            else :
                file1 = video_frames[local_idx - 1]
...
def main(args):
    ...
    if args.dataset == 'MICCAI2017':    
        ...
        filenames, _ = get_ACT_dataset_filenames(args)
    elif args.dataset == 'KPT':
        filenames, _ = get_KPT_dataset_filenames(args)
    else:
    ...
    for i, (filename, x_prev, x_curr) in enumerate(loader):
        ...
        case_dir = filename.parent.parent
        optflow_dir = case_dir / "opt_flow"
        optflow_dir.mkdir(exist_ok=True, parents=True)
        out_path = optflow_dir / (filename.stem + '.flo')
        write_flo(flow_uv, str(out_path))
        # print(f"write optical flow to {out_path}")
        ...
        if (args.visualize):
            ...
            optflow_vis_color_dir = case_dir / "opt_flow_vis"
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_dir', type=str, default='/data/home/hao/chenyan/data/training_act_data',
                        help='data directory.')
    parser.add_argument('--dataset', type=str, default='ACT', choices=['MICCAI2017', 'JIGSAWS', 'ACT', 'KPT'],
                        help='dataset name. e.g. MICCAI2017, JIGSAWS, ACT, KPT')
    ...
    # useless args for running this script, but needed to call get_XXX_dataset_filenames()
    parser.add_argument('--val_cases_count', type=int, default=1)
