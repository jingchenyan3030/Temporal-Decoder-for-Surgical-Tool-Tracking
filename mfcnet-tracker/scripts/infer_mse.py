...
# predicted heatmap:
def save_pred_heatmaps(
    hm, ori_img, save_dir, frame_name,
    ...
    for c, cname in enumerate(class_names):
        ...
        norm = np.clip((class_map - vmin) / (vmax - vmin), 0, 1)
        norm = (norm * 255).astype(np.uint8)

        color_hm = cv2.applyColorMap(norm, cv2.COLORMAP_VIRIDIS)

        cv2.imwrite(
            str(save_dir / f"{frame_name}_heatmap_abs_{cname}.png"),
            color_hm
        )

        overlay = cv2.addWeighted(ori_img, 0.7, color_hm, 0.3, 0)
        cv2.imwrite(
            str(save_dir / f"{frame_name}_overlay_abs_{cname}.png"),
...
def save_points_prediction(args, img_path, keypoints_dict, input_rgb):
    ...
    if args.dataset == 'ACT':
        ...
        out_dir = Path(args.data_dir).parent / "0923_test_multiframe_mse" / out_rel
    ...
    with torch.no_grad():
        for sample in dataloader: 
            ...
            for b in range(end - step):
                ...
                img_path = Path(file_names[step + b])
                ...
                if heatmap_parser is not None:
                    ...
                    if args.dataset == 'ACT':
                        ...
                        base_pred_root = Path(args.data_dir).parent / "0923_test_multiframe_mse"
