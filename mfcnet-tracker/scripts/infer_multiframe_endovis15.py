...
from utils.dataloader_utils import get_MICCAI2017_dataset_filenames, get_JIGSAWS_dataset_filenames, get_MICCAI2015_dataset_filenames, get_ACT_dataset_filenames
...
# Adding:
def save_overlay_prediction(args, img_path, pred_mask, input_rgb=None):
    """
    img_path: 当前 frame 对应原始图像路径 (str)
    pred_mask: HxW uint8 预测mask
    input_rgb: (可选) 已经反标准化的RGB图像 (HxWx3, uint8)，如果传入就不用再读硬盘
    """
    p = Path(img_path)
    rel = p.relative_to(args.data_dir)
    case_dir, video_dir = rel.parts[0], rel.parts[1]

    out_dir = Path(args.data_dir) / 'test_multiframe' / case_dir / video_dir / 'pred'
    ...
    overlay = img.copy()
    alpha = 0.5
    # 调色板：0=bg,1=tip,2=anchor,3=contact
    palette = np.array([[0,0,0],[255,255,0],[255,0,0],[0,255,0]], dtype=np.uint8)
    for cls in range(1,4):
        mask = (pred_mask == cls)
        overlay[mask] = (alpha*palette[cls] + (1-alpha)*overlay[mask]).astype(np.uint8)

    overlay_bgr = cv2.cvtColor(overlay, cv2.COLOR_RGB2BGR)
    out_file = out_dir / f"{p.stem}_overlay.png"
    cv2.imwrite(str(out_file), overlay_bgr)
    ...
    with torch.no_grad():
        for sample in dataloader: 
            ...
            if args.num_classes == 10:
                ...
                err, pres_gt, pres, c_gt, c_pred = centroid_error(torch.exp(output), mask, args)
...
# Adding:
                pred_mask = mask_array.astype(np.uint8)
                ori_img = input[0][0,:3,:,:].cpu().numpy()
                mean = np.array([0.485, 0.456, 0.406]).reshape((3,1,1))
                std = np.array([0.229, 0.224, 0.225]).reshape((3,1,1))
                ori_img = np.clip(ori_img *std + mean, 0, 1)
                ori_img = (np.transpose(ori_img, (1,2,0))*255).astype(np.uint8)
                save_overlay_prediction(args, file_names[step], pred_mask, input_rgb=ori_img)
