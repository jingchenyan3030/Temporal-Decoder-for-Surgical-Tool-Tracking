...
# Adding 1
def _mask_centroid(mask_unit8):
    ''' mask_unit8: H x W, np.uint8, 0/255, return: x, y'''
    ys, xs = np.where(mask_unit8==255)
    if len(xs)==0:
        return np.nan, np.nan
    return float(np.mean(xs)), float(np.mean(ys))

# Adding 2
def centroid_error_4_classes(output, gt,class_ids=(1,2,3)):
    if output.dim() == 4:
        pred_classes = output[0].detach().cpu().numpy().argmax(axis=0)
    else:
        pred_classes = output.detach().cpu().numpy().argmax(axis=0)
    gt_classes = output.detach().cpu().numpy() if gt.dim() == 4 else gt.cpu().numpy()
    errs, p_gt, p_pr, c_gt, c_pr = {}, {}, {}, {}, {}

    for cls in class_ids:
        pr_mask = (pred_classes==cls).astype(np.uint8)*255
        gt_mask = (gt_classes==cls).astype(np.uint8)*255

        x_pr, y_pr = _mask_centroid(pr_mask)
        x_gt, y_gt = _mask_centroid(gt_mask)
        c_gt[cls] = (x_gt, y_gt)
        c_pr[cls] = (x_pr, y_pr)

        p_gt[cls] = not np.isnan(x_gt) or not np.isnan(y_gt)
        p_pr[cls] = not np.isnan(x_pr) or not np.isnan(y_pr)

        if p_gt[cls] and p_pr[cls]:
            errs[cls] = float(np.sqrt((x_pr - x_gt)**2 + (y_pr - y_gt)**2))
        else:
            errs[cls] = np.nan
    return errs, p_gt, p_pr, c_gt, c_pr
...
def centroid_error(output, gt, args): 
    if args.num_classes != 5:
        if args.num_classes == 3:
            ...
            errs, p_gt, p_pr, c_gt, c_pr = centroid_error_4_classes(output, gt, class_ids=(1,2,3))
            return errs, p_gt, p_pr, c_gt, c_pr
        else: 
